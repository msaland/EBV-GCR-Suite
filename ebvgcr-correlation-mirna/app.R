...
ui <- fluidPage(title = "EBV-GCR - EBV miRNA vs. Cellular miRNA", theme = shinytheme("cosmo"), useShinyjs(),
  ...
  titlePanel("EBV-GCR - Correlation of EBV miRNA and Cellular miRNA Expression"),
  p(style="text-align: justify;", strong("Description: "), "The purpose of this analysis is to explore relationships between viral gene miRNA expression and expression of candidate cellular miRNAs in HEBV positive cervical gastric carcinomas (GC). Select the candidate cellular miRNAs by typing the name in the input box on the top right to perform a Spearman's correlation analysis. Generated figures can be resized via dragging the right or bottom figure edges, or via the grey triangle in the bottom right edge of the figure. Heat maps can be downloaded as PNG or PDF files and the significance matrix data can be downloaded as a CSV file. Level 3 miRNA expression data from the TCGA GC cohort was downloaded from the Broad Genome Data Analysis Center’s Firehose server", a(href="https://gdac.broadinstitute.org/", "Link"), ". EBV viral miRNA expression data were sourced from Ungerleider et al, PLOS Pathogens 2021 (EBV miRNAs are potent effectors of tumor cell transcriptome remodeling in promoting immune escape) ",a(href="https://pubmed.ncbi.nlm.nih.gov/33956915/", "DOI: 10.1371/journal.ppat.1009217")),
  p(style="text-align: justify;", strong("Download the Correlation Master List: "), a(href="Correlation-cell_miRNA-vs-vir_miRNA.xlsx", "Click Here", download=NA, target="_blank")),
...
server <- function(input, output, session) {
  ...
  output$tableGC <- renderDT({
    ...
    tablePre <- readRDS(paste("../ebvgcr-data/corr-stats-mirna/gc-", geneCell, ".rds", sep=""))
  ...
  drawPlot <- function(plot.data, titles, geneCell, mode) {
    ...
    pval=c()
    for(i in 5:41){
      var1=c(var1, gsub("hsa-","",colnames(plot.data[3])))
