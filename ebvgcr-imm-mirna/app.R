...
my.env <- new.env()
autofill1 <- sort(readRDS("../ebvgcr-data/autofill-miRNA.rds"))
...
ui <- fluidPage(title = "EBV-GCR - miRNA vs. Immune Function", theme = shinytheme("cosmo"), useShinyjs(),
  ...
  titlePanel("EBV-GCR - Correlation of Cellular miRNA Expression and Immune Function by EBV Status"),
  p(style="text-align: justify;", strong("Description: "), "The purpose of this analysis is to explore relationships between candidate cellular miRNA expression and various calculated characteristics related to the tumor immune landscape in Epstein-Barr virus (EBV) positive, microsatellite-unstable/instability (MSI), chromosomal instability (CIN), genomically stable (GS), and mutant polymerase epsilon (POLE) tumors in the TCGA gastric carcinoma (GC) cohort. Select the candidate cellular miRNAs by typing the name in the input box on the top right. Select the immune function/factor to analyze by selecting the desired option in the adjacent dropdown. Generated figures can be resized via dragging the right or bottom figure edges, or via the grey triangle in the bottom right edge of the figure. Correlation plots can be downloaded as PNG or PDF files and the information necessary to reproduce the box and scatter plots can be downloaded as a CSV file. Level 3 miRNA expression data from the TCGA GC cohort was downloaded from the Broad Genome Data Analysis Center’s Firehose server", a(href="https://gdac.broadinstitute.org/", "Link"), ". Immune landscape data was extracted from Thorsson et al, Immunity 2018", a(href="https://www.ncbi.nlm.nih.gov/pmc/articles/PMC5982584/", "DOI: 10.1016/j.immuni.2018.03.023")),
...
server <- function(input, output, session) {
  ...
  output$table <- renderDT({
    ...
    gc.CIN.gene.summary <- c(paste("GC CIN -", gene, sep=" "), as.numeric(count(gc.CIN.gene)$n), round(as.numeric(min(gc.CIN.gene)), digits = 3), round(as.numeric(max(gc.CIN.gene)), digits = 3), round(mean(unlist(gc.CIN.gene)), digits = 3), round(as.numeric(summarize_all(gc.CIN.gene, ~ quantile(.x, probs = 0.25))$Value), digits = 3), round(as.numeric(summarize_all(gc.CIN.gene, ~ quantile(.x, probs = 0.5))$Value), digits = 3), round(as.numeric(summarize_all(gc.CIN.gene, ~ quantile(.x, probs = 0.75))$Value), digits = 3))
    ...
    gc.MSI.gene.summary <- c(paste("GC MSI -", gene, sep=" "), as.numeric(count(gc.MSI.gene)$n), round(as.numeric(min(gc.MSI.gene)), digits = 3), round(as.numeric(max(gc.MSI.gene)), digits = 3), round(mean(unlist(gc.MSI.gene)), digits = 3), round(as.numeric(summarize_all(gc.MSI.gene, ~ quantile(.x, probs = 0.25))$Value), digits = 3), round(as.numeric(summarize_all(gc.MSI.gene, ~ quantile(.x, probs = 0.5))$Value), digits = 3), round(as.numeric(summarize_all(gc.MSI.gene, ~ quantile(.x, probs = 0.75))$Value), digits = 3))
    gc.GS.gene <- data.frame(na.omit(gc.combined[gc.combined$Status == "STAD_GS", gene]))
    names(gc.GS.gene)[1] = "Value"
    gc.GS.gene.summary <- c(paste("GC GS -", gene, sep=" "), as.numeric(count(gc.GS.gene)$n), round(as.numeric(min(gc.GS.gene)), digits = 3), round(as.numeric(max(gc.GS.gene)), digits = 3), round(mean(unlist(gc.GS.gene)), digits = 3), round(as.numeric(summarize_all(gc.GS.gene, ~ quantile(.x, probs = 0.25))$Value), digits = 3), round(as.numeric(summarize_all(gc.GS.gene, ~ quantile(.x, probs = 0.5))$Value), digits = 3), round(as.numeric(summarize_all(gc.GS.gene, ~ quantile(.x, probs = 0.75))$Value), digits = 3))
    gc.POLE.gene <- data.frame(na.omit(gc.combined[gc.combined$Status == "STAD_POLE", gene]))
    names(gc.POLE.gene)[1] = "Value"
    gc.POLE.gene.summary <- c(paste("GC POLE -", gene, sep=" "), as.numeric(count(gc.POLE.gene)$n), round(as.numeric(min(gc.POLE.gene)), digits = 3), round(as.numeric(max(gc.POLE.gene)), digits = 3), round(mean(unlist(gc.POLE.gene)), digits = 3), round(as.numeric(summarize_all(gc.POLE.gene, ~ quantile(.x, probs = 0.25))$Value), digits = 3), round(as.numeric(summarize_all(gc.POLE.gene, ~ quantile(.x, probs = 0.5))$Value), digits = 3), round(as.numeric(summarize_all(gc.POLE.gene, ~ quantile(.x, probs = 0.75))$Value), digits = 3))
    ...
    row.names(table) <- c(paste("GC CIN -", gsub("-.*","",gene), sep=" "), paste("GC EBV -", gsub("-.*","",gene), sep=" "), paste("GC MSI -", gsub("-.*","",gene), sep=" "), paste("GC GS -", gsub("-.*","",gene), sep=" "), paste("GC POLE -", gsub("-.*","",gene), sep=" "), paste("GC CIN -", gsub("-"," ",factor), sep=" "), paste("GC EBV -", gsub("-"," ",factor), sep=" "), paste("GC MSI -", gsub("-"," ",factor), sep=" "), paste("GC GS -", gsub("-"," ",factor), sep=" "), paste("GC POLE -", gsub("-"," ",factor), sep=" "))
  ...
  output$tableFactor <- renderDT({
    ...
    tablePR <- readRDS(paste("../ebvgcr-data/imm-stats-mirna/", gene, ".rds", sep=""))
    pval.CIN = c()
    pval.EBV = c()
    pval.MSI = c()
    pval.GS = c()
