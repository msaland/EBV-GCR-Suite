...
my.env <- new.env()
autofill <- sort(readRDS("../ebvgcr-data/autofill-mRNA.rds"))
...
ui <- fluidPage(title="EBV-GCR - mRNA vs. OS", theme = shinytheme("cosmo"), useShinyjs(),
  ...
  titlePanel("EBV-GCR - Correlation of Cellular mRNA Expression and Overal Survival by EBV Status"),
  p(style="text-align: justify;", strong("Description: "), "The purpose of this analysis is to explore relationships between expression of a candidate mRNA and patient overall survival (OS) between Epstein-Barr virus (EBV) positive, microsatellite-unstable/instability (MSI), chromosomal instability (CIN), genomically stable (GS), and mutant polymerase epsilon (POLE) tumors in gastric carcinoma (GC). Select the candidate mRNA by typing the name in the input box on the top right. Patients can be stratified into 2, 3 or 4 comparison groups based on expression levels of the target mRNA using the adjacent input box. Generated figures can be resized via dragging the right or bottom figure edges, or via the grey triangle in the bottom right edge of the figure. Kaplan-Meier plots can be downloaded as PNG or PDF files. Survival data was extracted from Liu et al, Cell 2018",a(href="https://www.ncbi.nlm.nih.gov/pmc/articles/PMC6066282/", "DOI: 10.1016/j.cell.2018.02.052")),
...
server <- function(input, output, session) {
  ...
  output$gcdownload3V <- downloadHandler(
    filename = function() {paste("GC-MSI-", my.env$g.gene, ".pdf", sep="")},
  ...
  drawPlot <- function(plot.data, gene, titles, mode, compare.G) {
    ...
    if (as.numeric(compare.G) == 2){
      my.plot <- ggsurvplot(surv.plot.data, data = plot.data.mut,  risk.table = TRUE, xlim = c(0,60), break.time.by = 12,  
                            xlab = "Time in months", ylab = "Percent Survival", legend.labs = c(paste("LOW", gsub('(.*)-\\w+','\\1',gene), sep=" "), paste("HIGH", gsub('(.*)-\\w+','\\1',gene), sep=" ")), title = titles, font.legend = list(size = 12, color = "black"))
    } else if (as.numeric(compare.G) == 3){
      my.plot <- ggsurvplot(surv.plot.data, data = plot.data.mut,  risk.table = TRUE, xlim = c(0,60), break.time.by = 12,  
                            xlab = "Time in months", ylab = "Percent Survival", legend.labs = c(paste("LOW", gsub('(.*)-\\w+','\\1',gene), sep=" "), paste("MID", gsub('(.*)-\\w+','\\1',gene), sep=" "), paste("HIGH", gsub('(.*)-\\w+','\\1',gene), sep=" ")), title = titles, font.legend = list(size = 12, color = "black"),)
    } else if (as.numeric(compare.G) == 4){
      my.plot <- ggsurvplot(surv.plot.data, data = plot.data.mut,  risk.table = TRUE, xlim = c(0,60), break.time.by = 12,  
                            xlab = "Time in months", ylab = "Percent Survival", legend.labs = c(paste("LOW", gsub('(.*)-\\w+','\\1',gene), sep=" "), paste("MID-LOW", gsub('(.*)-\\w+','\\1',gene), sep=" "), paste("HIGH-MID", gsub('(.*)-\\w+','\\1',gene), sep=" "), paste("HIGH", gsub('(.*)-\\w+','\\1',gene), sep=" ")), title = titles, font.legend = list(size = 12, color = "black"))
    }
    return(my.plot)
  }
