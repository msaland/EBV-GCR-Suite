...
my.env <- new.env()
autofillCell <- sort(readRDS("../ebvgcr-data/autofill-miRNA.rds"))
gc.EBV.genes <- readRDS("../ebvgcr-data/EBVaGC_vir_mRNA.rds")
colnames(gc.EBV.genes) <- gsub("\\.", "-", colnames(gc.EBV.genes))
EBV.miRNA <- sort(gsub("ebv-","",colnames(gc.EBV.genes)[3:ncol(gc.EBV.genes)]))

ui <- fluidPage(title = "EBV-GCR - EBV mRNA vs. Cellular miRNA", theme = shinytheme("cosmo"), useShinyjs(),
  ...
  titlePanel("EBV-GCR - Correlation of EBV mRNA and Cellular miRNA Expression"),
  p(style="text-align: justify;", strong("Description: "), "The purpose of this analysis is to explore relationships between viral gene mRNA expression and expression of candidate cellular miRNAs in EBV positive cervical gastric carcinomas (GC). Select the candidate cellular miRNAs by typing the name in the input box on the top right to perform a Spearman's correlation analysis. Generated figures can be resized via dragging the right or bottom figure edges, or via the grey triangle in the bottom right edge of the figure. Heat maps can be downloaded as PNG or PDF files and the significance matrix data can be downloaded as a CSV file. Level 3 miRNA expression data from the TCGA GC cohort was downloaded from the Broad Genome Data Analysis Center’s Firehose server", a(href="https://gdac.broadinstitute.org/", "Link"), ". EBV viral mRNA expression data were sourced from Chakravorty et al, Cancer Research 2019 (Integrated Pan-Cancer Map of EBV-Associated Neoplasms Reveals Functional Host-Virus Interactions) ",a(href="https://pubmed.ncbi.nlm.nih.gov/31481499/", "DOI: 10.1158/0008-5472.CAN-19-0615"),"."),
  p(style="text-align: justify;", strong("Download the Correlation Master List: "), a(href="Correlation-cell_miRNA-vs-vir_mRNA.xlsx", "Click Here", download=NA, target="_blank")),
...
server <- function(input, output, session) {
  ...
  output$tableGC <- renderDT({
    ...
    tablePre <- readRDS(paste("../ebvgcr-data/corr-stats-mmirna/gc-", geneCell, ".rds", sep=""))
    names=paste(gsub("hsa-","",geneCell),"vs.",rownames(tablePre),sep=" ")
    s=c()
    for (i in 1:19){
  ...
  drawPlot <- function(plot.data, titles, geneCell, mode) {
    ...
    pval=c()
    for(i in 5:23){
      var1=c(var1, gsub("-.*","",colnames(plot.data[3])))
      var2=c(var2, colnames(plot.data[i]))
    ...
    my.plot1 <- ggplot(data = data1, aes(var1, var2, fill = correl)) +
      ...
      scale_y_discrete(labels=EBV.miRNA[c(1:7)]) +
      scale_x_discrete(labels=gsub("hsa-","",geneCell)) +
    ...
    my.plot2 <- ggplot(data = data2, aes(var1, var2, fill = correl)) +
      geom_tile(color = "white") +
      scale_y_discrete(labels=EBV.miRNA[c(8:13)]) +
      scale_x_discrete(labels=gsub("hsa-","",geneCell)) +
      geom_text(aes(var1, var2, label = correl), color="black", size=5, vjust = -0.1) +
      geom_text(aes(label=pval), color="black", size=5, vjust = 0.9) + 
      scale_fill_gradient2(low = "blue", high = "red", mid = "white", midpoint = 0, limit = c(-1,1), space = "Lab", name="Spearman\nCorrelation") + theme_minimal() + 
    ...
    my.plot3 <- ggplot(data = data3, aes(var1, var2, fill = correl)) +
      geom_tile(color = "white") +
      scale_y_discrete(labels=EBV.miRNA[c(14:19)]) +
      scale_x_discrete(labels=gsub("hsa-","",geneCell)) +
      geom_text(aes(var1, var2, label = correl), color="black", size=5, vjust = -0.1) +
      geom_text(aes(label=pval), color="black", size=5, vjust = 0.9) + 
      scale_fill_gradient2(low = "blue", high = "red", mid = "white", midpoint = 0, limit = c(-1,1), space = "Lab", name="Spearman\nCorrelation") + theme_minimal() + 
  ...
  makePlotData <- function(geneCell, mode) {
    if (mode == "GC") {
      gc.mRNA <- readRDS(paste("../ebvgcr-data/mirna-genes/GC-", geneCell, ".rds", sep=""))
