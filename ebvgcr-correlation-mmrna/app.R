...
EBV.miRNA <- sort(colnames(gc.EBV.genes[3:ncol(gc.EBV.genes)]))

ui <- fluidPage(title = "EBV-GCR - EBV mRNA vs. Cellular mRNA", theme = shinytheme("cosmo"), useShinyjs(),
  ...
  titlePanel("EBV-GCR - Correlation of EBV mRNA and Cellular mRNA Expression"),
  p(style="text-align: justify;", strong("Description: "), "The purpose of this analysis is to explore relationships between viral gene mRNA expression and expression of candidate cellular mRNAs in EBV positive gastric carcinomas (GC). Select the candidate cellular mRNAs by typing the name in the input box on the top right to perform a Spearman's correlation analysis. Generated figures can be resized via dragging the right or bottom figure edges, or via the grey triangle in the bottom right edge of the figure. Heat maps can be downloaded as PNG or PDF files and the significance matrix data can be downloaded as a CSV file. Level 3 mRNA expression data from the TCGA GC cohort was downloaded from the Broad Genome Data Analysis Center’s Firehose server", a(href="https://gdac.broadinstitute.org/", "Link"), ". EBV viral mRNA expression data were sourced from Chakravorty et al, Cancer Research 2019 (Integrated Pan-Cancer Map of EBV-Associated Neoplasms Reveals Functional Host-Virus Interactions) ",a(href="https://pubmed.ncbi.nlm.nih.gov/31481499/", "DOI: 10.1158/0008-5472.CAN-19-0615"),"."),
  p(style="text-align: justify;", strong("Download the Correlation Master List: "), a(href="Correlation-cell_mRNA-vs-vir_mRNA.xlsx", "Click Here", download=NA, target="_blank")),
...
server <- function(input, output, session) {
  ...
  output$tableGC <- renderDT({
    ...
    tablePre <- readRDS(paste("../ebvgcr-data/corr-stats-mmrna/gc-", geneCell, ".rds", sep=""))
