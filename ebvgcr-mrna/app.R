...
ui <- fluidPage(title = "EBV-GCR - mRNA vs. EBV Status", theme = shinytheme("cosmo"), useShinyjs(), 
  ...
  fluidRow(style="margin-left: 1px;", titlePanel("EBV-GCR - Correlation of Cellular mRNA Expression and EBV Status")),
  p(style="text-align: justify;", strong("Description: "), "The purpose of this analysis is to compare expression of candidate cellular mRNAs between Epstein-Barr virus (EBV) positive, microsatellite-unstable/instability (MSI), chromosomal instability (CIN), genomically stable (GS), mutant polymerase epsilon (POLE), and Normal control tissue from the TCGA gastric carcinoma (GC) cohort. Select the candidate mRNA by typing the name in the input box on the top right. Generated figures can be resized via dragging the right or bottom figure edges, or via the grey triangle in the bottom right edge of the figure. Box plots can be downloaded as PNG or PDF files and the information necessary to reproduce the box plots can be downloaded as a CSV file. Level 3 mRNA expression data from the TCGA GC cohort was downloaded from the Broad Genome Data Analysis Center’s Firehose server", a(href="https://gdac.broadinstitute.org/", "Link"), ". "),
  ...
  p(style="text-align: justify;", strong("Video Guide: "), a(href="https://thincr.ca/ebvgcr-home/#v1", "Not Updated")),
...
server <- function(input, output, session) {
  ...
  output$boxPlotGC <- renderPlot({
    ...
    gc.plot <- drawPlot(plot.data, gene, paste("GC Dataset -",gsub('(.*)-\\w+','\\1',gene),sep=" "))
